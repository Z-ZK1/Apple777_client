  ...
  <div class="home_wrap">
    ...
    <form class="form">
      ...
      <div class="recharge_wrap" style="overflow: hidden;">
        <div :class="i == data.money ? 'chose_money fast-money':'fast-money'" v-for="(i, k) in recharge_list" @click="chmoney(i)" :key="k">
          {{ $t("钱符") }}{{ i }}
          <van-icon
            v-if="i == data.money"
            name="success"
          ...
          />
          <span class="recharge_bonus" v-show="getBonus(i) > 0">+{{ getBonus(i) }}</span>
        </div>
      </div>
